  ...
  return (
    <div className="prose prose-sm prose-invert max-w-none">
      ...
      <ul>
        ...
        <li>
          In this example, when fetching the data we return <code>notFound()</code> for{' '}
          <Link href="/not-found/does-not-exist">Categories</Link> and{' '}
          <Link href="/not-found/electronics/does-not-exist">Sub Categories</Link> that do not
          exist. This renders the closest appropriate <code>not-found.js</code>.
        </li>
        <li>
          <em>
            Note: <code>not-found.js</code> currently only renders when triggered by the{' '}
            <code>notFound()</code> function. We&apos;re working on support for catching unmatched
            routes (404).
          </em>
        </li>
      </ul>
